            	...
            	<div class="widget-content" style="border:0px;padding-top:5px;">
					...
					?>
								<li class="span4 thum_repositorio" data-dismiss="modal" 
									...
									onclick="usar_archivo('<?=$name_id?>','<?=$archivo?>');">
...
<?php
	...
	if($item == "mdv_videos")
	{
		...
		$archivos 		= Datos("mdv_archivos","tipo = 'video' order by fecha_ingreso DESC","*");
		...
		if(mysql_num_rows($archivos) > 0)
		{
			...
			$archivos       = Datos("mdv_archivos",
			                        "tipo = 'video' order by fecha_ingreso DESC limit ".
			                         $paginador['limitInf'].",".$paginador['tamPag'],
			                        "*");
		}
?>
			...
			<form action="#" class="form-horizontal">
            	<div class="widget-content" style="border:0px;padding-top:5px;">
            		...
            		<?php 
            			if(mysql_num_rows($archivos) == 0)
            			{
            				echo "<h4 style='width:100%; text-align:center;'>Aún no hay vídeos ingresados en el Sistema</h4>";
            			}
            		?>
					...
					<?php
						...
						$icono 			= BASEURL."img/film_icon.png";
					...
					?>
								<li class="span4 thum_repositorio" data-dismiss="modal" 
									style="background-image:url(<?=$icono;?>);background-size: 100% auto; border:0px;"
									onclick="usar_archivo('<?=$name_id?>','<?=$archivo?>');">
			...
			<form action="#" class="form-horizontal">
            	<div class="widget-content nopadding" style="border:0px;padding-top:5px;">
					...
					<?php
						...
						$dir 			= str_replace("/cph_admin", "", UPLOAD_RUTE)."/uploads";
						$base 			= str_replace("/cph_admin", "", BASEURL)."uploads/";
